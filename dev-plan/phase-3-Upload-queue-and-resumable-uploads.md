...
1. **Resume or open session.** If `existingSession` is non-null and not expired: call `provider.GetUploadedBytesAsync` to reconcile the offset against the stored `BytesUploaded` (taking the minimum), fall through to step 2. If null or expired (per `LastActivityUtc + provider TTL` — but the canonical signal is provider returning `UploadSessionExpired`): call `provider.BeginUploadAsync(remoteName, blob.EncryptedSize, ct)` and `UploadQueueRepository.GetOrCreateSessionAsync` to persist the returned session, fall through to step 2. The session's reconciled `BytesUploaded` is the offset the range loop in step 3 starts from; if it already equals `blob.EncryptedSize` (a crash between provider-confirmed last range and our finalise call), step 3 is a no-op and step 4 finalises immediately.
2. **Open the blob file** via `File.OpenRead` at the sharded path. Use `FileShare.Read` (Phase 5 self-healing may also need it).
3. **Range loop with per-range retry.** Rent one `IMemoryOwner<byte>` of `UploadConstants.RangeSize` from `MemoryPool<byte>.Shared` (Principle 19; reused across all ranges of this blob). For each offset in `[BytesUploaded, blob.EncryptedSize)`:
   - `Seek` and `ReadExactly` into the buffer.
   - **Inner per-range retry loop** (this is the §21.1 in-range ladder; lives here, not in `UploadQueueService`): start `rangeAttempt = 1`. Call `await provider.UploadRangeAsync(session, offset, buffer.Memory[..bytesRead], ct)`. Branch on the returned `Result`:
     - **Success** → `UpdateSessionProgressAsync(BytesUploaded += bytesRead)`, advance the outer loop, reset `rangeAttempt = 1` for the next range.
     - **`UploadSessionExpired`** → best-effort `AbortUploadAsync(CancellationToken.None)`, `DeleteSessionAsync(CancellationToken.None)`, return to step 1 from the top (open a fresh session, re-read the offset; the outer loop continues from the new `BytesUploaded`).
     - **Other retryable failure** (`ProviderUnreachable`, `ProviderRateLimited`, `Timeout`, `UploadFailed`, transient `HttpRequestException`-mapped codes): consult `RetryPolicy.NextRangeAttempt(rangeAttempt)`. If the decision is `Retry(delay)`: `await clock.Delay(delay, ct)`, increment `rangeAttempt`, retry the same range from the same offset. If the decision is `EscalateCycle`: return `UploadOutcome.RetryableFailure(errorCode)` to the caller — the per-cycle ladder lives in `UploadQueueService` (§3.4), which records the failure on `TailUploads`, schedules the next-cycle delay, and re-enters this method later. The buffer owner is disposed as the method exits.
     - **Permanent failure** (`ProviderAuthFailed`, `ProviderQuotaExceeded`, `TokenRevoked`, `ChecksumMismatch` from a downstream verification step): return `UploadOutcome.PermanentFailure(errorCode)` immediately — no retry, no cycle escalation. The caller marks the row `FAILED` and publishes Critical/Error.
     - **Cancelled** (`OperationCanceledException`): propagate by re-throwing inside the catch-OperationCanceledException-first block (Principle 14), which becomes `Result.Fail(Cancelled)` at the public boundary; the caller sees this as orchestrator/volume shutdown and the `UploadSessions` row is preserved for the next session to resume from.
...
- `RetryableFailure`: this is the outcome `RangeUploader` returns when its inner `RetryPolicy.NextRangeAttempt` ladder decided `EscalateCycle` (i.e. the in-range 3-retry budget — §21.1 first row — was exhausted on the offending range; see §3.3 step 3). Per-range retries already happened *inside* `RangeUploader.UploadAsync`; the worker's job here is the *cycle* ladder. `MarkFailedAsync` with the pending error code, consult `RetryPolicy.NextCycleAttempt(row.AttemptCount)`, then `await clock.Delay(decision.Delay, ct)` for the cycle backoff (5 min / 30 min / 2 h / 12 h per §21.1) and loop back to dequeue. The per-row `AttemptCount` column on `TailUploads` (incremented by `MarkUploadingAsync`) is the cycle counter the worker reads on resume — it persists across process restarts so a crash mid-cycle doesn't reset the ladder. When `NextCycleAttempt` returns `MarkFailed` (5th cycle), fall through to the `PermanentFailure` case.
...
- The lifecycle is asymmetric on `DisposeAsync`: brain mirror runs **before** the upload queue stops. Note: the brain mirror does **not** route through `UploadQueueService` — it calls `IStorageProvider` methods directly (§3.5 step 4), resolving each provider through the same `IProviderRegistry` the queue uses. The reason ordering matters is shared *queue-side* state, not a queue-routing dependency:
    1. **`volumeCts` is shared.** Step 1 cancels it. The brain mirror's final-mirror call deliberately uses `CancellationToken.None` for the upload itself (Principle 17 — compensation must not be cancellable mid-flight), so cancellation alone doesn't stop it; but if a worker is mid-`UploadRangeAsync` against the same provider and that call is observing `volumeCts`, an HTTP client / connection pool hit by abort can disrupt a sibling request. Letting the mirror run first lets it grab the provider while connection state is clean.
    2. **Per-tail in-flight upload contention.** A worker uploading a 4 MB range to provider X and the brain-mirror finalisation pushing `_brain/{ts}.bin` to the same provider compete for whatever per-provider serialization the adapter implements (some providers serialise per-account, some per-session). Running the final mirror before signalling workers to drain avoids this contention with no extra coordination.
    3. **The brain `SqliteConnection` lives on the volume**, so the mirror's `BackupDatabase` snapshot is unaffected by queue disposal — disposal of the connection itself happens later in step 4 of the lifecycle.
  Document with an XML comment on the dispose method explaining (1)–(2). If a future refactor moves brain-mirror uploads to route through `UploadQueueService` (one possibility for V2 if mirror retries want the same backoff ladder), this rationale changes to "the mirror has nowhere to land if the queue is stopped" and the comment should be updated accordingly.
