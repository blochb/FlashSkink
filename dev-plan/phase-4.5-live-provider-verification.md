...
xUnit v2 runs distinct test classes (collections) in parallel by default. The three provider classes talk to three distinct vendor accounts, hold no shared mutable static state (each builds its own provider from its own cache entry), and write under fully-quarantined per-test prefixes (decision 5) — so cross-class parallel execution is correct, and is left enabled because it also keeps the suite fast. Within a class, xUnit runs methods sequentially; we do **not** enable in-class parallelization. **The shipped default applies no shared `[Collection]` attribute, so the three classes are in separate collections and run in parallel.** To serialize the whole set — when a developer is rate-limited or deliberately points more than one class at a single shared account — apply a shared `[Collection("LiveProvider")]` to all three classes; in xUnit v2 a shared collection name disables their mutual parallelism. Serialization is a performance/throttling lever, never a correctness requirement; the README documents the switch.
...
| §4.5.3 | OneDrive live verification | `OneDriveLiveTests` wiring the harness to `OneDriveSetup`/`OneDriveProvider` (`createUploadSession` + PUT-range, server-confirmed `nextExpectedRanges` resume, session expiry). OneDrive does not expose `ISupportsRemoteHashCheck` (QuickXorHash exists in code for future use but is unwired today), so integrity rests on the GCM tag + round-trip. Any OneDrive adapter fixes + recorded-fake regression guards. |
...
`tests/FlashSkink.Tests/Providers/Live/GoogleDriveLiveTests.cs` — class `GoogleDriveLiveTests` (class-level `[Trait("Category","LiveProvider")]`; no shared `[Collection]` by default — decisions 1 and 10):
...
**Scope summary:** New `DropboxLiveTests` reusing `LiveProviderHarness`, bound to `DropboxSetup`/`DropboxProvider`. Credentials: `FLASHSKINK_LIVE_DROPBOX_CLIENTID/_CLIENTSECRET`. Like Google Drive (§4.5.1 D1), Dropbox deliberately does **not** implement `ISupportsRemoteHashCheck` — its `ContentHash` is a SHA-256 digest, structurally incompatible with the `ulong` XXHash64 the interface returns, and is logged at finalisation only — so there is no contract surface to query a remote hash. Its integrity building block is therefore the upload→download→byte-identical round-trip (GCM-tag-backed), and the hash test is the cheap negative fact `Dropbox_RemoteHashCheck_NotSupported_ByDesign`. The resume building block exercises `upload_session/start`→`append_v2`→`finish` and confirms `GetUploadedBytesAsync` reports the server-confirmed offset; Dropbox's 48 h session TTL is a known boundary the resume test stays well within. The harness is reused **unchanged** (it already ships no hash exercise after §4.5.1 D1). Any Dropbox adapter fix lands here with a recorded-fake regression guard (decision 6).
...
**Scope summary:** New `OneDriveLiveTests` reusing `LiveProviderHarness`, bound to `OneDriveSetup`/`OneDriveProvider`. Credentials: `FLASHSKINK_LIVE_ONEDRIVE_CLIENTID/_CLIENTSECRET`. Like Google Drive (§4.5.1 D1) and Dropbox (§4.5.2 D1-Dropbox), OneDrive deliberately does **not** implement `ISupportsRemoteHashCheck`: a QuickXorHash implementation exists in the codebase (`OneDriveQuickXorHash`) for future use, but it is **not exposed through `ISupportsRemoteHashCheck` today**, so there is no contract surface to query a remote hash and **the live tests do not verify quickXorHash/SHA1**. Its integrity building block is therefore the upload→download→byte-identical round-trip (GCM-tag-backed), and the hash test is the negative fact `OneDrive_RemoteHashCheck_NotSupported_ByDesign`. The resume building block exercises `createUploadSession` + ranged `PUT` and confirms resume from the server-confirmed `nextExpectedRanges`/`GetUploadedBytesAsync` offset; OneDrive session expiry (~7 days) is noted as a known boundary. **No fixed redirect port is needed** — Microsoft identity ignores the loopback port, so the ephemeral-port default applies (unlike Dropbox, §4.5.2). The harness is reused **unchanged**. Any OneDrive adapter fix lands here with a recorded-fake regression guard (decision 6).

**Test spec:** `OneDriveLiveTests` with the seven `OneDrive_*` tests in the §4.5.1 shape — setup, upload/download round-trip, resume `[Theory]` `(3,1)/(3,2)/(4,3)`, `RemoteHashCheck_NotSupported_ByDesign`, exists/delete/list, health/quota/used, and the opt-in `PurgeAllLiveTestObjects` (gated on `FLASHSKINK_LIVE_PURGE`).
