...
public sealed class BrainBackedProviderRegistry : IProviderRegistry, IAsyncDisposable
{
    ...
    {
        return CreateAsync(brain, dek, new GoogleDriveClientFactory(), new DropboxClientFactory(), loggerFactory, ct);
    }

    /// <summary>
    /// Back-compat test overload — defaults the Dropbox factory to the production implementation.
    /// Retained so §4.3 Drive-only tests don't need to thread a Dropbox factory they don't use.
    /// </summary>
    internal static Task<Result<BrainBackedProviderRegistry>> CreateAsync(
        ...
        CancellationToken ct)
    {
        return CreateAsync(brain, dek, googleDriveFactory, new DropboxClientFactory(), loggerFactory, ct);
    }
        ...
        CancellationToken ct)
    {
        switch (row.ProviderType)
        {
            ...
            case "dropbox":
                return await TryBuildDropboxAdapterAsync(
        ...
        CancellationToken ct)
    {
        ...
        {
            logger.LogWarning(
                "Dropbox provider row '{ProviderId}' has no ClientID; skipping.", row.ProviderID);
            return null;
        }
        if (row.EncryptedToken is null || row.EncryptedToken.Length == 0)
        {
            logger.LogWarning(
                "Dropbox provider row '{ProviderId}' has no EncryptedToken; skipping.", row.ProviderID);
            return null;
        }
        if (row.EncryptedClientSecret is null || row.EncryptedClientSecret.Length == 0)
        {
            logger.LogWarning(
                "Dropbox provider row '{ProviderId}' has no EncryptedClientSecret; skipping.", row.ProviderID);
            return null;
        }
        ...
        {
            logger.LogWarning(
                "Dropbox provider row '{ProviderId}': could not decrypt client secret; skipping.",
                row.ProviderID);
            return null;
        }
        if (!ProviderTokenCrypto.TryDecrypt(row.EncryptedToken, dek, out var refreshToken))
        {
            logger.LogWarning(
                "Dropbox provider row '{ProviderId}': could not decrypt refresh token; skipping.",
                row.ProviderID);
            return null;
        }

        // Optional persisted root path. Dropbox creates intermediate folders implicitly on upload,
        // so a missing config just means "use the default root" — the setup helper handles it.
        ...
        if (!string.IsNullOrWhiteSpace(row.ProviderConfig))
        {
            ...
            {
                logger.LogWarning(ex,
                    "Dropbox provider row '{ProviderId}' has malformed ProviderConfig; skipping.",
                    row.ProviderID);
                return null;
            }
        }

        // Token-exchange HTTP client is unused on the CreateProviderFromConfigAsync path; the
        // setup constructor still requires one (mirror of Drive pattern). using-disposed below.
