...
35. **Single-instance per volume.** `FlashSkinkVolume.CreateAsync` and `OpenAsync` acquire an OS-level exclusive file lock on `[skinkRoot]/.flashskink/instance.lock` (`FileStream` with `FileShare.None` — the only mode that's reliably exclusive on both Windows kernel sharing checks and Linux `flock(LOCK_EX)`) before any vault, brain, or staging work. The holder identity (pid, host, started-at, app-version) lives in a sibling file `[skinkRoot]/.flashskink/instance.manifest` so peers can read it without contending on the exclusion file. A second concurrent open on the same root — same host or a different host against a network-shared USB — returns `Result.Fail(ErrorCode.SingleInstanceLockHeld)` with metadata identifying the holder (`Pid`, `Host`, `StartedAtUtc`, `AppVersion`). The lock is released as the final step of `DisposeAsync`, after the upload queue, brain mirror, and `VolumeSession` have all torn down — holding it across the entire shutdown window keeps a concurrent second-instance launch correctly blocked until teardown completes. Stale-lock files left behind by a crashed process are recoverable via `VolumeCreationOptions.ForceUnlock` (the CLI `--force` flag lands in Phase 4); the flag never seizes a live lock. (Blueprint §19.5)
